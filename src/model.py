...
class AdversarialFakeNewsModel(nn.Module):
    def __init__(self, model_name='xlm-roberta-base', num_classes=1, num_sources=101, lightweight=False, source_dropout=0.5):
        ...
        fusion_dim = 768 + 32 if self.lightweight else 1312
        num_gates = 2 if self.lightweight else 5
        self.gating_network = nn.Sequential(
            nn.Linear(fusion_dim, num_gates),
            nn.Softmax(dim=1)
        )
    ...
    def forward(self, input_ids, attention_mask, images=None, audio_feats=None, video_feats=None, knowledge_feats=None, source_ids=None, alpha=1.0, mc_dropout=False):
        ...
        # Apply source dropout during training to resolve shortcut memorization
        if self.training and source_ids is not None and self.source_dropout > 0.0:
            random_mask = torch.rand(source_ids.size(), device=source_ids.device) < self.source_dropout
            source_ids = torch.where(random_mask, torch.tensor(100, device=source_ids.device), source_ids)
        ...
        if self.lightweight:
            raw_combined = torch.cat((pooled_output, source_feats), dim=1)
            gate_weights = self.gating_network(raw_combined) # [batch, 2]
            combined_features = raw_combined * gate_weights.repeat_interleave(torch.tensor([768, 32]).to(pooled_output.device), dim=1)
        else:
